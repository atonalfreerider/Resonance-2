...
public class HarmonyExplorer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        VisualElement panel=controls;

        var common=new VisualElement{name="side-transport"};
        focusHint=Label(common,"Click the torus to play / orbit. Click this panel to edit.");
        ...
        var sharedTransport=Row(common);sharedTransport.AddToClassList("shared-transport");
        var play=new Button(()=>{if(midi.IsPlaying)midi.Pause();else midi.Play();}){text="Play",name="side-play-pause"};sharedTransport.Add(play);
        Button(sharedTransport,"Stop",midi.Stop);
        ...
        var structure=tabs.AddPage("structure","Song structure");
        var advanced=tabs.AddPage("advanced","Advanced settings");
        loading.Add(recording.BuildUI());
        solo.Add(GetComponent<StemPlayback>().BuildUI());
        Slider(solo,"Master volume",0,1,main.Synth.Volume,v=>main.Synth.Volume=v);
        ...
        Section(panel,"LIVE MIDI INPUT");
        Choice(panel,"Live MIDI input (Windows)",LiveMidiInput.Devices(),0,i=>live.Connect(i-1));
        var midiFiles=new Foldout{text="Prepared MIDI preview",value=false};loading.Add(midiFiles);
        path=new TextField("File path"){value=midi!=null?midi.midiPath:""};midiFiles.Add(path);
        Button(midiFiles,"Open MIDI…",OpenMidi);
        Button(midiFiles,"Load path",()=>{StopLesson(); midi?.Load(path.value);});
        midiStatus=Label(midiFiles,"");
        ...
        Section(structure,"HARMONIC HISTORY"); historyLabel=Label(structure,"");
        Button(structure,"Export progression",Export);

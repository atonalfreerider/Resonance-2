...
public static class HarmonicOutlineValidation
{
    public static async void Run(){
        ...
        var results=new List<string>();double position=midi.Position;bool playing=midi.IsPlaying;bool midiEnabled=midi.enabled;
        ...
        try{
            ...
            foreach(var test in new[]{(name:"major",root:0,third:4),(name:"minor",root:0,third:3),(name:"secondary dominant",root:2,third:4),(name:"Neapolitan",root:1,third:4)}){
                ...
                for(int i=0;i<3;i++){main.ChordOutlinePath(pcs[i],pcs[(i+1)%3],path);Check(Vector3.Distance(lines[i].GetPosition(0),path[0])<.013f&&Vector3.Distance(lines[i].GetPosition(40),path[40])<.013f,"Outer-register surface endpoints: "+test.name);}
            }
            results.Add("ALL GEAR / CHORD OUTLINE CHECKS PASSED");
        }catch(Exception e){results.Add("FAIL: "+e);}
        finally{main.Silence();midi.enabled=midiEnabled;midi.Seek(position);if(playing)midi.Play();File.WriteAllLines("Temp/ResonanceChecks/harmonic-outline.txt",results);}
    }
}

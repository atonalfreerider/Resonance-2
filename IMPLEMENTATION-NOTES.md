...
Visual lifecycle repair: SurfaceOverlay, DrumPatternDeck and DominantChordOutline now build and release their owned render objects on enable/disable. Missing MIDI timing hides the drum deck safely; every pooled drum pin receives bounded size and dim material properties before activation. Active chord regions use a 24-step tessellated surface patch with 19% chord-color tint and matching shortest-path boundaries. Pattern note/trail colors come from the saved chord at note onset, independent of instrument/channel; the selected lead vocal is white. Validation: VisualLifecycleValidation covers three enable/disable cycles, initial pin size/brightness, missing timing recovery, all 24 major/minor fill boundaries and wheel colors; 785 model checks also pass.

Chord-phase persistence: the region outline and tint now use the prepared chord interval at the recording-clock position, including while paused. Opacity is fixed; boundaries switch immediately, and rests/undefined intervals clear the region. Live/manual mode retains chord recognition as fallback. Released vibration lines desaturate toward dark gray (rate 8/s) and lose extra light (rate 4/s) without shortening the surface route. ChordPhaseValidation verifies silent persistence, fixed tint, immediate phase/rest changes, reverse seeks, and sub-20% glow after 450ms.

Portable Windows release: Tools/PatternPrep now emits gap-free RegionPhases covering every section; raw Chords remain unchanged for analysis. Region visibility reads this context timeline, so sparse notes and rest observations cannot blank the section. PortableWindowsBuild packages the x64 player with PreparedSongs/TicketToRide next to the executable; standalone startup prefers that bundled score over machine-specific preferences. PortableSmokeCheck runs only with --portable-smoke-test and verifies relocated paths, recording-clock advancement, and visible regions across every section.

Windows portable validation passed from Builds/PortableCheck, using the bundled WAV and 5,279-note MIDI in A major. Recording sample-clock advancement and visible regions at five positions in every section passed with zero runtime errors. Standalone requires explicit URP Unlit shader inclusion and a saved HarmonyPanelSettings resource containing ICU/UI text resources; both are now included. Final ZIP: Builds/Resonance-Windows-TicketToRide.zip (87,381,375 bytes), CRC validated.
